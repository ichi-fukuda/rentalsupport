...
export async function POST(req: NextRequest) {
  ...
  const vehicle = typeof vehicleId === "string" ? getVehicle(vehicleId) : undefined;
  if (!vehicle) {
  ...
  const system =
    kind === "fuel"
      ? "You are a multilingual assistant for a Japanese rental car company. " +
        ...
        "Be concrete and step-by-step. Mention any Japan-specific conventions (e.g. gas station attendant language, " +
        "self-service pump colors, CHAdeMO connectors) only if relevant to the source notes. " +
