...
export async function POST(req: NextRequest) {
  ...
  const vehicle =
    typeof vehicleId === "string"
      ? await db.vehicle.findUnique({ where: { id: vehicleId }, include: { host: true } })
      : null;
  if (!vehicle) {
  ...
  const shopNotes = vehicle.host.accidentNotes.trim();
  ...
  try {
    const response = await client.messages.create({
      ...
      system:
        ...
        (shopNotes ? `\n\nAdditional shop-specific instructions to include: ${shopNotes}` : ""),
      messages: [
        {
          role: "user",
